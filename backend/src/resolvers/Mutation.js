...
const Mutations = {
  async createItem(parent, args, ctx, info) {
    ...
    const item = await ctx.db.mutation.createItem(
      {
        data: {
          // create relationship btwn The Item and the User
          user: {
            connect: {
  ...
  async signup(parent, args, ctx, info) {
    ...
    ctx.response.cookie("token", token, {
      httpOnly: true,
      maxAge: 1000 * 60 * 60 * 24 * 365 // set 1 year cookie
    });
  ...
  async requestReset(parent, { email, password }, ctx, info) {
    ...
    // 2. Set a reset and expiry on that user
    const randomBytesPromisified = promisify(randomBytes);
    const resetToken = (await randomBytesPromisified(25)).toString("hex");
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    const password = await bcrypt.hash(args.password, 10);
    // 5. Save the new password to the user and remov old resetToken fields
  ...
  async updatePermissions(parent, args, ctx, info) {
    ...
    hasPermission(currentUser, ["ADMIN", "PERMISSIONUPDATE", "BOSS"]);
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    // 3. Query the users current cart
